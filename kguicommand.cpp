...
#include "kguicommand.moc"

KGuiCmdManager *KGuiCmdManager::s_pSelf = 0;
...
KGuiCmdManager *KGuiCmdManager::self()
{
  if ( !s_pSelf )
    s_pSelf = new KGuiCmdManager;
...
bool KGuiCmdDispatcher::eventFilter(QObject *, QEvent *e) {
  ...
  if (enabled && e->type() == QEvent::KeyPress /*Accel*/) {    // key press
