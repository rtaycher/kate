...
void KantDocument::insertLine( const QString &, int )
{
}

void KantDocument::insertAt( const QString &s, int line, int col, bool  )
{
  VConfig c;
  c.view = 0; // ### FIXME
  c.cursor.x = col;
...
void KantDocument::removeLine( int  )
{
}

int KantDocument::length() const
{
  return text().length();
}

void KantDocument::setSelection( int , int , int , int )
{
}

bool KantDocument::hasSelection() const
{
  return false;
}

QString KantDocument::selection() const
{
  return QString::null;
}

int KantDocument::numLines() const
{
#ifdef NEW_CODE
  return buffer->count();
#else
...
TextLine::Ptr KantDocument::getTextLine(int line) const {
#ifdef NEW_CODE
...
int KantDocument::textLength(int line) {
  TextLine::Ptr textLine = getTextLine(line);
  if (!textLine) return 0;
  return textLine->length();
}

void KantDocument::setTabWidth(int chars) {
...
void KantDocument::setReadWrite( bool rw )
{
  setReadOnly( !rw );
  KTextEditor::Document::setReadWrite( rw );
}

bool KantDocument::isReadWrite() const
{
  return !isReadOnly();
}

void KantDocument::setReadOnly(bool m) {
...
//    if (readOnly) recordReset();
    for (view = m_views.first(); view != 0L; view = m_views.next() ) {
      emit static_cast<KantView *>( view )->newStatus();
    }
  }
}

bool KantDocument::isReadOnly() const {
  return readOnly;
}

void KantDocument::setNewDoc( bool m )
{
//  KTextEditor::View *view;
...
bool KantDocument::isNewDoc() const {
  return newDoc;
}

void KantDocument::setModified(bool m) {
  KTextEditor::View *view;
...
bool KantDocument::isModified() const {
  return modified;
}

void KantDocument::readConfig(KConfig *config) {
  int z;
  char s[16];
...
void KantDocument::writeConfig(KConfig *config) {
  int z;
  char s[16];
...
void KantDocument::readSessionConfig(KConfig *config) {
  readConfig(config);
...
void KantDocument::writeSessionConfig(KConfig *config) {
...
void KantDocument::setHighlight(int n) {
  Highlight *h;
...
void KantDocument::updateFontData() {
...
void KantDocument::hlChanged() { //slot
  makeAttribs();
  updateViews();
}


void KantDocument::addView(KTextEditor::View *view) {
  views.append( static_cast<KantView *>( view ) );
  KTextEditor::Document::addView( view );
  connect( static_cast<KantView *>( view ), SIGNAL( destroyed() ), this, SLOT( slotViewDestroyed() ) );
}

void KantDocument::removeView(KTextEditor::View *view) {
//  if (undoView == view) recordReset();
  disconnect( static_cast<KantView *>( view ), SIGNAL( destroyed() ), this, SLOT( slotViewDestroyed() ) );
  views.removeRef( static_cast<KantView *>( view ) );
  KTextEditor::Document::removeView( view );
}

void KantDocument::slotViewDestroyed()
{
  views.removeRef( static_cast<const KantView *>( sender() ) );
}

bool KantDocument::ownedView(KantView *view) {
  // do we own the given view?
  return (views.containsRef(view) > 0);
}

bool KantDocument::isLastView(int numViews) {
  return ((int) views.count() == numViews);
}

int KantDocument::textWidth(const TextLine::Ptr &textLine, int cursorX) {
  int x;
  int z;
  QChar ch;
  Attribute *a;
...
int KantDocument::textWidth(PointStruc &cursor) {
  if (cursor.x < 0)
     cursor.x = 0;
  if (cursor.y < 0)
     cursor.y = 0;
...
int KantDocument::textWidth(bool wrapCursor, PointStruc &cursor, int xPos) {
  int len;
  int x, oldX;
  int z;
  QChar ch;
...
int KantDocument::textPos(const TextLine::Ptr &textLine, int xPos) {
/*  int newXPos;

  return textPos(textLine, xPos, newXPos);
}

int KantDocument::textPos(const TextLine::Ptr &textLine, int xPos, int &newXPos) {
*/
//  int len;
  int x, oldX;
  int z;
...
int KantDocument::textWidth() {
  return maxLength + 8;
}

int KantDocument::textHeight() {
  return numLines()*fontHeight;
}

void KantDocument::insert(VConfig &c, const QString &s) {
  int pos;
  QChar ch;
  QString buf;
...
#ifdef NEW_CODE
void KantDocument::loadFile(const QString &file, QTextCodec *codec)
{
  buffer->insertFile(0, file, codec);
qWarning("Linecount = %d (loadFile)", buffer->count());
}

void KantDocument::appendData(const QByteArray &data, QTextCodec *codec)
{
...
bool KantDocument::writeFile(const QString &file, QTextCodec *codec)
{
qWarning("writeFile()");
  QFile f( file );
  ...
  while(true)
  {
    ...
    if (eolMode != KantView::eolUnix) stream << QChar('\r');
    if (eolMode != KantView::eolMacintosh) stream << QChar('\n');
  };
  f.close();
  return (f.status() == IO_Ok);
}
#else
void KantDocument::loadFile(QIODevice &dev) {
  QChar ch;
  char last = '\0';
  char s;
  ...
  while ( !stream.eof() ) {
      ...
      if (ch.isPrint() || s == '\t') {
        ...
        if (last != '\r' || s != '\n') {
          ...
          if (s == '\r') eolMode = KantView::eolMacintosh;
        } else eolMode = KantView::eolDos;
        last = s;
      }
  }
//  updateLines();
...
void KantDocument::writeFile(QIODevice &dev) {
  ...
  do {
    ...
    if (eolMode != KantView::eolUnix) dev.putch('\r');
    if (eolMode != KantView::eolMacintosh) dev.putch('\n');
  } while (true);
}
#endif

int KantDocument::currentColumn(PointStruc &cursor) {
  return getTextLine(cursor.y)->cursorX(cursor.x,tabChars);
}

bool KantDocument::insertChars(VConfig &c, const QString &chars) {
  int z, pos, l;
  bool onlySpaces;
  QChar ch;
  QString buf;
  ...
  for (z = 0; z < (int) chars.length(); z++) {
    ch = chars[z];
    if (ch == '\t' && c.flags & KantView::cfReplaceTabs) {
      ...
      if (c.flags & KantView::cfAutoBrackets) {
  ...
  if (c.flags &KantView:: cfDelOnInput) delMarkedText(c);
...
/*  //do nothing if spaces will be removed
  ...
  if (c.flags & KantView::cfWordWrap && c.wrapAt > 0) {
    int line;
    const QChar *s;
//    int pos;
...
void KantDocument::newLine(VConfig &c) {
...
void KantDocument::backspace(VConfig &c) {
...
void KantDocument::del(VConfig &c) {
  TextLine::Ptr textLine = getTextLine(c.cursor.y);
...
void KantDocument::cut(VConfig &c) {
...
void KantDocument::copy(int flags) {
...
void KantDocument::paste(VConfig &c) {
...
void KantDocument::toggleRect(int start, int end, int x1, int x2) {
  int z, line;
  bool t;
...
void KantDocument::selectTo(VConfig &c, PointStruc &cursor, int cXPos) {
  ...
  if (c.cursor.x != select.x || c.cursor.y != select.y) {
    ...
    if (!(c.flags & KantView::cfKeepSelection)) deselectAll();
//      else recordReset();
...
//    tagLines(y, ye);
    ...
    if (c.flags & KantView::cfXorSelect) {
...
void KantDocument::invertSelection() {
  TextLine::Ptr textLine;
...
void KantDocument::selectWord(PointStruc &cursor, int flags) {
  int start, end, len;
  ...
  if (end <= start) return;
  if (!(flags & KantView::cfKeepSelection)) deselectAll();
//    else recordReset();
...
void KantDocument::doIndent(VConfig &c, int change) {
  ...
  if (selectEnd < selectStart) {
    ...
    if (c.flags & KantView::cfKeepIndentProfile && change < 0) {
  ...
  recordEnd(c.view, c.cursor, c.flags | KantView::cfPersistent);
}
...
*/
void KantDocument::optimizeLeadingSpace(int line, int flags, int change) {
  int len;
  ...
  for (chars = 0; chars < len; chars++) {
    ...
    if (ch == ' ') {
      space++;
      if (flags & KantView::cfSpaceIndent && okLen == chars) okLen++;
    } else if (ch == '\t') {
  ...
  if (flags & KantView::cfKeepExtraSpaces) chars -= extra;

  if (flags & KantView::cfSpaceIndent) {
    space -= extra;
    ch = ' ';
  } else {
    space /= tabChars;
...
void KantDocument::doCommentLine(PointStruc &cursor) {
...
void KantDocument::doUncommentLine(PointStruc &cursor) {
...
void KantDocument::doComment(VConfig &c, int change) {
...
QString KantDocument::text() const {
  TextLine::List::ConstIterator it = contents.begin();
  QString s;
...
QString KantDocument::getWord(PointStruc &cursor) {
  int start, end, len;
...
void KantDocument::setText(const QString &s) {
  int pos;
  QChar ch;
...
QString KantDocument::markedText(int flags) {
  TextLine::Ptr textLine;
  int len, z, start, end, i;

  len = 1;
  if (!(flags & KantView::cfVerticalSelect)) {
    for (z = selectStart; z <= selectEnd; z++) {
      textLine = getTextLine(z);
...
void KantDocument::delMarkedText(VConfig &c/*, bool undo*/) {
  int end = 0;
...
void KantDocument::tagLineRange(int line, int x1, int x2) {
  int z;
...
void KantDocument::tagLines(int start, int end) {
  int z;
...
void KantDocument::updateLines(int startLine, int endLine, int flags, int cursorY) {
  TextLine::Ptr textLine;
  int line, last_line;
  int ctxNum, endCtx;
  ...
  do {
    ...
    if (line <= endLine && line != cursorY) {
      if (flags & KantView::cfRemoveSpaces) textLine->removeSpaces();
...
void KantDocument::updateMaxLength(TextLine::Ptr &textLine) {
  int len;
...
void KantDocument::slotBufferChanged() {
  newDocGeometry = true;
  updateViews();
}

void KantDocument::updateViews(KantView *exclude) {
  KantView *view;
  int flags;
  bool markState = hasMarkedText();

  flags = (newDocGeometry) ? KantView::ufDocGeometry : 0;
  for (view = views.first(); view != 0L; view = views.next() ) {
...
QColor &KantDocument::cursorCol(int x, int y) {
  int attr;
  Attribute *a;
...
QFont &KantDocument::getTextFont(int x, int y) {
  int attr;
  Attribute *a;
...
void KantDocument::paintTextLine(QPainter &paint, int line, int xStart, int xEnd,
  bool showTabs) {
...
void KantDocument::setURL( const KURL &url, bool updateHighlight )
{
  ...
  m_url = url;
  for (view = m_views.first(); view != 0L; view = m_views.next() ) {
    emit static_cast<KantView *>( view )->fileChanged();
  }
...
void KantDocument::clearFileName() {
  KTextEditor::View *view;
...
bool KantDocument::doSearch(SConfig &sc, const QString &searchFor) {
  int line, col;
  int searchEnd;
  int bufLen, tlen;
  QChar *t;
  ...
  line = sc.cursor.y;
  col = sc.cursor.x;
  if (!(sc.flags & KantView::sfBackward)) {
    //forward search
    if (sc.flags & KantView::sfSelected) {
      if (line < selectStart) {
        line = selectStart;
        col = 0;
      }
    ...
    while (line <= searchEnd) {
      ...
      QString text(t, tlen);
      if (sc.flags & KantView::sfWholeWords) {
  ...
  sc.flags |= KantView::sfWrapped;
  return false;
found:
  if (sc.flags & KantView::sfWrapped) {
    if ((line > sc.startCursor.y || (line == sc.startCursor.y && col >= sc.startCursor.x))
      ^ ((sc.flags & KantView::sfBackward) != 0)) return false;
  }
  sc.cursor.x = col;
  sc.cursor.y = line;
  return true;
...
void KantDocument::markFound(PointStruc &cursor, int len) {
//  unmarkFound();
//  recordReset();
  if (foundLine != -1) {
    getTextLine(foundLine)->unmarkFound();
...
void KantDocument::tagLine(int line) {

  if (tagStart > line) tagStart = line;
  if (tagEnd < line) tagEnd = line;
}

void KantDocument::insLine(int line) {
  KantView *view;
...
void KantDocument::delLine(int line) {
  KantView *view;
...
void KantDocument::optimizeSelection() {
  TextLine::Ptr textLine;
...
void KantDocument::newUndo() {
  ...
  undoState = state;
  for (view = m_views.first(); view != 0L; view = m_views.next() ) {
    emit static_cast<KantView *>( view )->newUndo();
  }
}

void KantDocument::recordStart(VConfig &c, int newUndoType) {
...
//  }
  ...
  g = undoList.getLast();
  if (g != 0L && ((undoCount < 1024 && flags & KantView::cfGroupUndo
...
void KantDocument::recordInsert(VConfig &c, const QString &text) {
  recordReplace(c, 0, text);
}

void KantDocument::recordReplace(VConfig &c, int len, const QString &text) {
  if (c.cursor.x > 0 && !(c.flags & KantView::cfSpaceIndent)) {
    TextLine::Ptr textLine = getTextLine(c.cursor.y);
...
void KantDocument::recordInsert(PointStruc &cursor, const QString &text) {
  recordReplace(cursor, 0, text);
}

void KantDocument::recordDelete(PointStruc &cursor, int len) {
...
void KantDocument::recordEnd(VConfig &c) {
...
/*
void KantDocument::recordDel(PointStruc &cursor, TextLine::Ptr &textLine, int l) {
  int len;
...
void KantDocument::undoTypeList(QValueList<int> &lst)
{
  lst.clear();
...
void KantDocument::redoTypeList(QValueList<int> &lst)
{
  lst.clear();
...
void KantDocument::setUndoSteps(int steps) {
  if (steps < 5) steps = 5;
  undoSteps = steps;
}

void KantDocument::setPseudoModal(QWidget *w) {
...
void KantDocument::newBracketMark(PointStruc &cursor, BracketMark &bm) {
  TextLine::Ptr textLine;
...
void KantDocument::clipboardChanged() { //slot
//#if defined(_WS_X11_)
  if (m_singleSelection) {
...
void KantDocument::guiActivateEvent( KParts::GUIActivateEvent *ev )
{
