...
{ 
  ...
  if (delLen+pos > textLen) 
    delLen = textLen-pos; 
 
  uint oldLen = textLen; 
  textLen = textLen-delLen+insLen; 
  ...
  for (uint m=pos+delLen; m < textLen; m++)  
  {  
    if (m+newAtStuff >= textLen) break;  
    if (m >= textLen) break;  
  
    text[m+newAtStuff]=text[m];  
    attributes[m+newAtStuff]=attributes[m];  
  }  
 
  for (uint m7=pos; m7 < pos+insLen; m7++)  
  {  
    if (m7 < textLen) text[m7]=insText[m7-pos];  
  }  
  ...
  {  
    ...
    {  
      if (m2 < textLen) attributes[m2]=insAttribs[m2-pos];  
    }  
  }  
