...
QList<KantPartDocument> docList; //documents


TopLevel::TopLevel (KantPartDocument *doc, const QString &path)
{
  ...
  if (!doc) {
    doc = new KantPartDocument (false, false); //new doc with default path
    docList.append(doc);
  }
  setupEditWidget(doc);
  setupActions();
...
//  createShellGUI( false );
  if (kWrite->isLastView()) docList.remove((KantPartDocument*)kWrite->doc());
}
...
void TopLevel::setupEditWidget(KantPartDocument *doc)
{
  kWrite = new KantPartView(doc, this, 0, false);
...
{
  TopLevel *t = new TopLevel((KantPartDocument *)kWrite->doc());
  t->readConfig();
...
void TopLevel::newStatus()
{
  ...
  if (readOnly)
    ...
    statusBar()->changeItem(config & KantPartView::cfOvr ? i18n(" OVR ") : i18n(" INS "),ID_INS_OVR);
...
void TopLevel::restore(KConfig *config, int n)
{
  if (kWrite->isLastView() && !kWrite->doc()->url().isEmpty()) { //in this case first view
    loadURL(kWrite->doc()->url(), KantPartView::lfNewFile );
  }
...
{
  writeConfig(config);
  config->writeEntry("DocumentNumber",docList.find((KantPartDocument *)kWrite->doc()) + 1);
...
void TopLevel::saveGlobalProperties(KConfig *config) //save documents
{
  ...
  for (z = 1; z <= (int) docList.count(); z++) {
     ...
     config->setGroup(buf);
     doc = (KantPartDocument *) docList.at(z - 1);
...
void restore()
{
  ...
  for (z = 1; z <= docs; z++) {
     sprintf(buf,"Document%d",z);
     config->setGroup(buf);
     doc = new KantPartDocument (0L,0L);
...
{
  KCmdLineArgs::init( argc, argv, KantPartFactory::aboutData() );
