  ...
  //  Radiate error messages back to kate
...
ShowErrorMessage ( int argc, char** argv, QString strFileName, int nLine, QString strMessage  )
{
	...
	KApplication app( argc, argv, "KateClient", false );
	kapp->dcopClient()->attach();
	QByteArray data;
	QCString strCommand ("ShowErrorMessage(QString, int, QString)");
	QCString strObject ("KateIface");
	QCString strApp ("kate");
...
main ( int argc, char** argv )
{
	...
	ShowErrorMessage ( argc, argv, "katemainwindow.cpp", 42, "I dislike this line for some strange reason" );
